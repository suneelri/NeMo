...
def mtp_block_spec(config: "GPTConfig", vp_stage: Optional[int] = None) -> Optional[ModuleSpec]:
    ...
    if getattr(config, "mtp_num_layers", None):
        ...
        print("mtp_block_spec:", isinstance(config.transformer_layer_spec, Callable), inspect.signature(config.transformer_layer_spec).parameters)
        ...
        print("mtp_block_spec: spec: ", spec)

        if hasattr(spec, 'layer_specs') and len(spec.layer_specs) == 0:
            spec = default_layer_spec(config, vp_stage=vp_stage) 

        print("mtp_block_spec: default: ", spec)
